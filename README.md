...
- [x] Accessibility alt-text pass (2026-07-08) — every image on the site now has descriptive EN/RU alt text (the Wix export had empty alts or upload filenames/Facebook IDs); drafted by Claude from the images themselves, so a native-speaker review for names and places wouldn't hurt
...
- [x] Non-video content gaps closed (2026-07-09) — the two project pages whose Russian versions 404'd on Wix itself (`kopiya-curatorial-projects`, `kopiya-procartistination-branches`) now have Claude-translated RU versions built from the intact English pages; `poeticaltexts` needed nothing (it was a Wix hub page whose role the Texts section fills). Also cleaned two Wix artifacts on those pages: a stray «Заголовок 1» placeholder (the extraction script now filters editor placeholders) and the internal page title "Procartistination Branches_short" → "Procartistination Brunches". The 3 dead 2018 blog posts remain recoverable only from the Wix dashboard.
...
5. Only cancel the Wix subscription after everything is verified live. **Before cancelling**: (a) re-run `python3 scripts/refetch_originals.py` once more — it pulls full-res originals from the Wix CDN and stops working when Wix goes away; (b) run `./scripts/backup_mirror.sh` — it tars `mirror/` (1.5 GB, gitignored, exists only on this machine) into a checksummed archive; move the archive somewhere durable (external drive / cloud storage). It is the only offline backup of the Wix originals, including the two big lecture videos.
